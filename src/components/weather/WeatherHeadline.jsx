import { useContext } from "react"
import PinIcon from "../../assets/pin.svg"
import { LocationContext, WeatherContext } from "../../contexts"

import CloudIcon from "../../assets/cloud.svg";
import HazeIcon from "../../assets/haze.svg";
import SnowIcon from "../../assets/icons/snow.svg";
import SunnyIcon from "../../assets/icons/sunny.svg";
import RainIcon from "../../assets/rainy.svg";
import ThunderIcon from "../../assets/thunder.svg";
import getDateAndTime from "../../utils/getDateAndTime";

export default function WeatherHeadline() {
    const {weatherData} = useContext(WeatherContext);
    const {location, time, weather, temperature} = weatherData;
    const {location: searchLocation} = useContext(LocationContext);

    const getFinalLocation = () => {
        if (searchLocation.location === "") {
            return location;
        } else {
            return searchLocation.location
        }
    }

    const finalLocation = getFinalLocation();

    function getWeatherImage(weather) {
        switch(weather) {
            case "Rain":
                return RainIcon;
            case "Clouds":
                return CloudIcon;
            case "Clear":
                return SunnyIcon;
            case "Snow":
                return SnowIcon;
            case "Thunder":
                return ThunderIcon;
            case "Fog":
                return HazeIcon;
            case "Haze":
                return HazeIcon;
            case "Mist":
                return HazeIcon;
            default:
                return SunnyIcon
        }
    }

    const weatherImage = getWeatherImage(weather);
    
    return (
        <div>
            <div className="max-md:flex items-center justify-between md:-mt-10">
                <img src={weatherImage} alt="cloud" />
                <div className="max-md:flex items-center max-md:space-x-4">
                    <h1 className="text-[60px] lg:text-[80px] xl:text-[100px] leading-none md:mb-4">{Math.round(temperature)}°</h1>
                    <div className="flex items-center space-x-4 md:mb-4">
                        <img src={PinIcon} />
                        <h2 className="text-2xl lg:text-[50px]">{finalLocation}</h2>
                    </div>
                </div>
            </div>
            <p className="text-sm lg:text-lg">{getDateAndTime(time, false)}</p>
        </div>
    )
}
